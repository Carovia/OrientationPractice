...
class VOCDataset(torch.utils.data.Dataset):
    ...
    def _read_image(self, image_id):
        image_file = os.path.join(self.data_dir, "JPEGImages", "%s" % image_id)
        try:
            image = Image.open("%s.jpg" % image_file).convert("RGB")
        except FileNotFoundError:
            try:
                image = Image.open("%s.png" % image_file).convert("RGB")
            except FileNotFoundError:
                image = Image.open("%s.jpeg" % image_file).convert("RGB")
